...
function CardMain (props: CardMainProps):JSX.Element {
  ...
  return (
    <div className='cities__places-list places__list tabs__content'>
      { offerHouse.map( (house, idoffer) => {
        const keyValue = `${house.src + idoffer}`;
        ...
        return (
          <article
            ...
            key={ keyValue }
            onMouseMove = {() => {
              setBookmark((prevArr) => ([...prevArr.slice(0,idoffer), true, ...prevArr.slice(idoffer + 1)]));
            }}
            onMouseLeave = {() => {
              setBookmark((prevArr) => ([...prevArr.slice(0,idoffer), false, ...prevArr.slice(idoffer + 1)]));
            }}
          >
            { house.special &&
            ...
            <div className='cities__image-wrapper place-card__image-wrapper'>
              <a href='#'>
                <img
                  ...
                  src={`${house.src}`} width='260' height='200' alt='Place image'
                />
              </a>
            </div>
